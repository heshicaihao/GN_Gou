...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T>
     ...
     * Pull-to-Refresh can happen. An example of this state is when the Adapter View is scrolled to the top
     * and the mode is set to {@link Mode#PULL_FROM_START}. The default value is <var>true</var> if
     * {@link PullToRefreshBase#isPullToRefreshOverScrollEnabled() isPullToRefreshOverScrollEnabled()} returns
     * false.
     * 
    ...
    /**
     * Pass-through method for {@link PullToRefreshBase#getRefreshableView() getRefreshableView()}.
     * {@link AdapterView#setOnItemClickListener(OnItemClickListener) setOnItemClickListener(listener)}. This
     ...
     * Pull-to-Refresh can happen. An example of this state is when the Adapter View is scrolled to the top
     * and the mode is set to {@link Mode#PULL_FROM_START}
     * 
     * @param showIndicator
    ...
    private boolean isFirstItemVisible() {
        ...
        if (null == adapter || adapter.isEmpty()) {
             ...
             * PtRListView internally use a HeaderView which messes the positions up. For now we'll just add
             * one to account for it and rely on the inner condition which checks getTop().
             */
    ...
    private boolean isLastItemVisible() {
        ...
        if (null == adapter || adapter.isEmpty()) {
             ...
             * internally uses a FooterView which messes the positions up. For me we'll just subtract one to
             * account for it and rely on the inner condition which checks getBottom().
             */
