	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
				if (!(mRefreshableView instanceof ScrollView && ((ScrollView) mRefreshableView)
						.getChildAt(0) instanceof MultiColumnListView)) {
					return true;
				}
			}
			if (mRefreshableView instanceof ScrollView
					&& ((ScrollView) mRefreshableView).getChildAt(0) instanceof MultiColumnListView) {
				View child = ((ScrollView) mRefreshableView).getChildAt(0);
				if (child instanceof MultiColumnListView) {
					int top = ((MultiColumnListView) child).getScrollChildTop();
					int buttom = ((MultiColumnListView) child)
							.getScrollChildBottom();
					BaseAdapter adapater = (BaseAdapter) ((MultiColumnListView) child)
							.getAdapter();
//					Log.i("kkkkkk", y + "  " + mLastMoveY);
					if (y - mLastMoveY > 0) {
						if (adapater instanceof ImageGridAdapter
								&& ((ImageGridAdapter) adapater)
										.getmIsEditState()) {
					...
					} else if (y - mLastMoveY < 0) {
						// 表示上拉
						// Log.i("kkkkkk", buttom + "  "+child.getHeight());
						if (buttom > child.getHeight()) {
							mLastMoveY = y;
							return super.onInterceptTouchEvent(event);
						}
						mLastMoveY = y;
			...
			// Log.i("kkkkkk", "是否准备："+isReadyForPull());
			if (isReadyForPull()) {
	...
	public void showNoMoreText() {
		mFooterLayout.showNoMoreTextview();
	}

	public void hideNoMoreText() {
		mFooterLayout.hideNoMoreTextview();
	}

	public void hideFootview() {
		mFooterLayout.hideFootview();
	}
	...
	final void setState(State state, final boolean... params) {
		...
		case MANUAL_REFRESHING:
			Log.i("resetImpl", "MANUAL_REFRESHING:" + params[0]);
	...
	// if (viewState == State.REFRESHING || viewState ==
