...
//        String strResult = read(response.getEntity().getContent());
        String strResult = getJsonStringFromGZIP(response);
    ...
    private static String getJsonStringFromGZIP(HttpResponse response) {
        ...
        try {
            ...
            if (result != -1 && headerData == 0x1f8b) {
                LogUtils.logd(TAG, LogUtils.getThreadName() + " use GZIPInputStream  ");
                is = new GZIPInputStream(bis);
            } else {
                LogUtils.logd(TAG, LogUtils.getThreadName() + " not use GZIPInputStream");
                is = bis;
            }
...
//        LogUtils.logd(TAG, "getJsonStringFromGZIP net output : " + jsonString);
        return jsonString;
    }
