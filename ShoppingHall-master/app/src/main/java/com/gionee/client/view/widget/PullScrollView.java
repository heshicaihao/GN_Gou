...
public class PullScrollView extends ScrollView {
    /** 阻尼系数,越小阻力就越大. */
    private float mScrollRatio = 0.25f;
    ...
    private int mInitTopFirst;
    private int mInitButtomFirst;
    private boolean mIsFirstToZero = true;// 用来控制有的手机重复执行onScrollChanged时getScrollY()=0的情况
    private int mLastTop = -1;

    /**
     * @return the topViewListener
     */
    ...
    /**
     * @param topViewListener
     *            the topViewListener to set
     */
    ...
    public interface TopViewListener {
        public void setViewIsShow(boolean isShow);// 设置视图是否显示

        public void setFaceSizeDelta(int scrollY);// 根据ScrollView的位置设置头像的大小
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (null != attrs) {
            ...
            if (ta != null) {
                ...
                mHeaderVisibleHeight = (int) ta.getDimension(R.styleable.PullScrollView_headerVisibleHeight,
                        -1);
                ta.recycle();
            }
        }
    ...
    protected void onScrollChanged(int l, final int t, int oldl, int oldt) {
        ...
        if (getScrollY() == 0 && mIsFirstToZero) {
            mIsTop = true;
            mIsFirstToZero = false;
        }
        if (mTopViewListener != null) {
            if (t >= 0) {
                if (t != mLastTop) {
//                    Log.i("tttttt", mInitTopFirst + "   " + mInitButtomFirst + "   " + t);
                    if (mIsTop) {
                        // TODO Auto-generated method stub
                        mCurrentTop = mInitTop = mHeader.getTop();
                        mCurrentBottom = mInitBottom = mHeader.getBottom();
                    }
                    mHeader.layout(mHeader.getLeft(), mInitTopFirst - t, mHeader.getRight(), mInitButtomFirst
                            - t);
                    mLastTop = t;
                }
//                topViewListener.setViewIsShow(true);
            } else {
//                topViewListener.setViewIsShow(false);
            }
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            mCurrentBottom = mInitBottom = mHeader.getBottom();
            if (mInitTopFirst == 0) {
                mInitTopFirst = mInitTop;
            }
            if (mInitButtomFirst == 0) {
                mInitButtomFirst = mInitBottom;
            }
        }
...
//        isTop = false;
    ...
    private void doActionMove(MotionEvent event) {
        ...
        if (getScrollY() == 0) {
            ...
            mTopViewListener.setViewIsShow(false);
        }
        ...
        if (mIsMoving) {
            ...
            float contentMoveHeight = deltaY * mScrollRatio;
            // 计算header移动距离(手势移动的距离*阻尼系数*0.5)
            float scale = mContext.getResources().getDisplayMetrics().density;
            float maxMoveHeight = 140 * scale / 3;
            if (contentMoveHeight <= maxMoveHeight) {
                float headerMoveHeight = contentMoveHeight;
    ...
    private boolean isNeedAnimation() {
        return !mContentRect.isEmpty() && mIsMoving;
    }
